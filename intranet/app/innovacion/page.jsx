...
export default function Innovacion() {
    ...
    useEffect(() => {
        Promise.all([
            fetch("https://pokeapi.co/api/v2/pokemon/venusaur").then(res => res.json()),
            fetch("https://rickandmortyapi.com/api/character/?name=rick").then(res => res.json()),
            fetch("https://www.swapi.tech/api/people/3").then(res => res.json()),
        ])
        .then((info) => {
            setData(info)
            console.log(info)
    ...
    <div className={styles.container}>
        <h1 className={styles.title}>Ingeniería en Inovacion</h1>
        <p className={styles.ruta}>
        Inicio / Facultades / Ingeniería / Innovacion
        </p>
    ...
    <div className={styles.grid}>
        ...
        <div className={styles.card}>
            <AsyncAwait link="https://pokeapi.co/api/v2/pokemon/ditto" />
        </div>

        <div className={styles.card}>
            <PromiseThen link="https://pokeapi.co/api/v2/pokemon/charizard"/>
        </div>
